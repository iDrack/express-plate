 ...
 * @returns 
 */
async updateUser(id: number, newData: {name: string, email: string, role: string }): Promise<User> {
...
async updatePassword(id: number, oldPassword: string, newPassword: string): Promise<User> {
    if(oldPassword === newPassword) {
        throw new AppError("New password cannot be the same as old one.", 405);
    }
    const userToUpdate = await this.getUserById(id);
